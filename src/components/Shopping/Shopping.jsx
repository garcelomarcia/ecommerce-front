...
const Shopping = () => {
  ...
  const eliminarProducto = (el, cantidad) => {
    ...
    axios
      .delete(
        `https://ecommerce-back-3kra.onrender.com/api/cart/delete/${id}/${el.id}`
      )
  ...
  const buyCart = async () => {
    await axios.put(
      `https://ecommerce-back-3kra.onrender.com/api/cart/checkout/${id}`
    );
    axios(
      `https://ecommerce-back-3kra.onrender.com/api/sendMail/mailer/${id}`
    ).then((result) => {
      Swal.fire({
        title: result.data,
        imageUrl:
  ...
  useEffect(() => {
    axios
      .get(`https://ecommerce-back-3kra.onrender.com/api/cart/${id}`)
      .then((product) => {
        console.log(product);
        if (product.data !== "") {
