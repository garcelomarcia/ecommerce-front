...
function IndividualView() {
  ...
  useEffect(() => {
    axios(
      `https://ecommerce-back-3kra.onrender.com/api/products/styles/${style}/${color}/${size}`
    ).then((res) => setData(res.data));
  }, [style, color, size]);

  useEffect(() => {
    axios(
      `https://ecommerce-back-3kra.onrender.com/api/products/colors/${style}`
    ).then((res) => {
      setColorsAvailable(res.data);
    });
    axios(
      `https://ecommerce-back-3kra.onrender.com/api/products/sizes/${style}`
    ).then((res) => {
  ...
  const addToCart = () => {
    const { id } = userLoged;
    axios
      .post(`https://ecommerce-back-3kra.onrender.com/api/cart/add/${id}`, {
        data,
        url,
        quantity,
      })
